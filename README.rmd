...
SA_bonds_clean
#Pipe to kable to look nice in report
...
usdzar %>% ggplot() + geom_line(aes(date, Price, color = Name)) +
                     labs(x = "date", y = "USDZAR Exchange Rate", 
...
J400ReturnPlot <- T40Indexes %>% ggplot() + geom_line(aes(date, J400Return, color = J400Return))
J400ReturnPlot
```
...
J200ReturnPlot <- T40Indexes %>% ggplot() + geom_line(aes(date, J200Return, color = J200Return))
J200ReturnPlot
...
Cumreturns <- T40Indexes %>% ggplot() +
                            ...
                            geom_line(aes(date, J400Return, color = "blue"))+ 
                             labs(x = "date", y = "Index Returns", 
                            title = "Index Returns for the J200 & J400")
...
PCA allows the modeler a means of identifying structure in the covariance / correlation matrix to locate low-dimensional subspaces containing most of the variation in the data. 

Let’s ensure our data is mean centered, as well as scaled, when doing the PCA (this is important, as our returns data are typically skew, non-normal and non-zero meaned. Thus a log transformation, scaling and mean-centering is standard practice).


```{r}
library(zoo)

J200IndexReturn <- T40 %>% arrange(date) %>% select(date, Tickers, Return, J200) %>% drop_na() %>% select(date, Tickers, Return)
...
fmxdat::theme_fmx(axis.size.title = fmxdat::ggpts(30), axis.size = fmxdat::ggpts(30), 
    title.size = fmxdat::ggpts(42), CustomCaption = T) + 
scale_y_continuous(breaks = scales::pretty_breaks(10), labels = scales::percent_format(accuracy = 1)) + 
...
```{r}
J400IndexReturn <- T40 %>% arrange(date) %>% select(date, Tickers, Return, J400) %>% drop_na() %>% select(date, Tickers, Return)
...
```{r}
Currencydataset<- cncy %>% arrange(date) %>% spread(Name, Price) 
...
ZARCrncy <- Currencydataset %>% select(date, `SouthAfrica `)

ZARCrncy 
```

```{r}
ZarCurrPlot <- ZARCrncy %>% ggplot() + geom_line(aes(date, `SouthAfrica `))+
                             labs(x = "date", y = "ZAR USD Exchange", 
                            title = "ZAR USD Exchange rate ")
...
Tidyrtn <- rtn %>% tbl_xts(., cols_to_xts = "dlogret") %>% PerformanceAnalytics::Return.clean(., method = "boudt")
...
```
Look at carry trade as to why var widened
...
stockreturns <- msci %>% filter(Name %in% "MSCI_ACWI") %>% 
...
```{r}
g1 <- ggplot(dcc.time.var.cor %>% filter(grepl("MSCI_ACWI", Pairs), 
    !grepl("MSCI_ACWI", Pairs))) + geom_line(aes(x = date, y = Rho, 
    colour = Pairs)) + theme_hc() + ggtitle("Dynamic Conditional Correlations: MSCI_ACWI")
